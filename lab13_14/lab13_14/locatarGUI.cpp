...
void GUIcos::reincarca(const vector<Locatar>& locatari) {
	cos->clear();
...
void GUIbloc::connect_signal_slots() {
	...
	QObject::connect(lst, &QListWidget::itemSelectionChanged, [&]() {
		...
		QListWidgetItem* item_selectat = lst->selectedItems().at(0);
		QString nume = item_selectat->text();
		txt_nume->setText(nume);
		QString nr_ap_q = item_selectat->data(Qt::UserRole).toString();
	...
	QObject::connect(tab, &QTableWidget::itemSelectionChanged, [&]() {
		if (tab->selectedItems().isEmpty()) {
			txt_nr_ap->setText("");
			txt_nume->setText("");
			txt_suprafata->setText("");
			txt_tip->setText("");
		...
		QTableWidgetItem* item_selectat = tab->selectedItems().at(0);
		QString nume = item_selectat->text();
		txt_nume->setText(nume);
		QString nr_ap_q = item_selectat->data(Qt::UserRole).toString();
		txt_nr_ap->setText(nr_ap_q);
	...
	QObject::connect(btn_cos, &QPushButton::clicked, [&]() {
		auto cos = new GUIcos{ adm, srv };
		cos->show();
		});
	QObject::connect(btn_cosReadOnly, &QPushButton::clicked, [&]() {
		auto cosRO = new GUIcosReadOnly{ adm, srv };
		cosRO->show();
		});
	
}

void GUIbloc::reincarca_lista(const std::vector<Locatar>& locatari) {
	lst->clear();
	for (const auto& l : locatari) {
		QListWidgetItem* item = new QListWidgetItem(QString::fromStdString(l.get_nume()), lst);
		string nr_ap_s = std::to_string(l.get_nr_ap());
		item->setData(Qt::UserRole, QString::fromStdString(nr_ap_s));
	}
}

void GUIbloc::reincarca_tabel(const std::vector<Locatar>& locatari) {
	tab->clear();
	tab->setRowCount(int(srv.get_all_locatari().size()));
	tab->setColumnCount(1);
	QStringList headt;
	headt << "Nume:";
	tab->setHorizontalHeaderLabels(headt);
	tab->horizontalHeader()->setDefaultAlignment(Qt::AlignLeft);
	int row = 0;
	int column = 0;
	for (const auto& l : locatari) {
		QTableWidgetItem* item = new QTableWidgetItem(tr(l.get_nume().c_str()).arg((row)*column));
		tab->setItem(row, column, item);
		item->setData(Qt::UserRole, QString::number(l.get_nr_ap()));
		row++;
	}
}

void GUIbloc::adauga_butoane_din() {
